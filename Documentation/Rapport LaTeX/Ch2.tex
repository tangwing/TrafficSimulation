IDM est un modèle qui est basé sur l'état, c'est-à-dire quand on précise un moment, on peut déterminer les états (position, vitesse...) des voiture, et donc on peut afficher les voiture sur l'écran. Ce que l'on a besoin, c'est seulement une boucle où on renover les positions des voitures sur l'écran. A partir de cette idée, nous avons établi deux structures nécéssaires et l'algorithme principale.
...
nouvelle structure Config
{
    ...
    /** (Longeur(m) virtuel de la route/windowWidth(pixel). */
...
L'algorithme suivant est la partie du coeur, qui applique le modèle IDM. C'est un sous-algorithme plus détaillé sur le méthode pour mettre à jour le paramètre de l'accélération.
